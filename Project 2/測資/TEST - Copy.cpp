...
void Error_1( char unreconized_ch, int row, int col ) {
  int errorRow = row - gNewLine + 1 ;
  gNewLine = row + 1 ;
  if ( col == 0 && !gPreIsError )
    errorRow-- ;
  gPreIsError = false ;
  cout << "> Line " << errorRow ;
...
public:
  ...
  TokenInfo TrulyGetToken() {
    ...
    if ( mCh == EOF ) {
      ...
      if ( tempToken.mStr == "." )
        Error_1( '.', mRow, mCol ) ;  // a dot is an unreconized token
      return tempToken ;
    } // else if ()
    else if ( mCh == 39 ) {
    ...
    for ( int i = 0 ; i < mVecOfReDefId.size() ; i++ )
      for ( int j = 0 ; j < mVecOfId.size() ; j++ )
        if ( mVecOfId[ j ].mName == mVecOfReDefId[ i ].mName ) {
          ...
          mVecOfId[ j ].mType = mVecOfReDefId[ i ].mIsArray ;
          mVecOfId[ j ].mType = mVecOfReDefId[ i ].mSize ;
        }  // if ()
  ...
  void Execute_statement() {
    ...
    for ( int i = 0 ; i < mVecOfSyntaxResult.size() ; i++ ) {
      if ( mVecOfSyntaxResult[ i ].mStr == "Done" && mVecOfSyntaxResult[ i + 1 ].mStr == "(" &&
        ...
        mVecOfSyntaxResult[ i + 1 ].mStr == "(" && mVecOfSyntaxResult.size() == 5 ) {
